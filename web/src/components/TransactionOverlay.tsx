...
import { Loader2, Wallet, CheckCircle2, XCircle, FileText } from "lucide-react";
...
const STAGE_COPY: Record<Exclude<TxStage, "error">, { headline: string; detail: string }> = {
  ...
  "awaiting-signature": { headline: "Check your wallet",       detail: "Look for the Freighter popup and approve it to continue." },
...
export function TransactionOverlay({
  open, stage, title, errorMessage, onClose,
}: {
  open: boolean;
  stage: TxStage;
  title: string;
      ...
      <div className="w-full max-w-sm rounded-2xl border border-white/8 bg-surface p-8 text-center shadow-[0_24px_64px_-8px_rgba(0,0,0,0.8)] animate-in zoom-in-95 fade-in duration-200">
        ...
        {stage === "error" ? (
          <>
            ...
            <p className="mt-2 text-sm text-muted-foreground leading-relaxed">{errorMessage ?? "Please try again."}</p>
            <button
              onClick={onClose}
              className="mt-6 inline-flex h-10 items-center justify-center rounded-lg border border-white/8 px-5 text-sm font-semibold font-display text-foreground transition-all hover:bg-surface-raised hover:border-white/16"
            >
              Close
            </button>
          </>
          ...
          <>
            <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-primary/10">
              {stage === "awaiting-signature" ? (
                <Wallet className="h-7 w-7 animate-pulse text-primary" />
              ) : stage === "submitting" ? (
              ...
              {STEP_ORDER.map((s) => (
                <span
                  ...
                  className={cn(
                    ...
                    s === stage ? "w-6 bg-primary" : "w-1.5 bg-white/15"
                  )}
                />
              ))}
            </div>
